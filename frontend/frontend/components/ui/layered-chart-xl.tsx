...
// Sample data for FunnelChart (referred to as simpleFunnelData in source)
// Assuming similar structure, but can be customized
const RAW_FUNNEL_DATA: FunnelDataPoint[] = [
  { key: 'Visitors', data: 1500 },
  { key: 'Sign-ups', data: 1200 },
  { key: 'Activations', data: 900 },
  { key: 'Paid Users', data: 400 },
  { key: 'Renewals', data: 300 },
  { key: 'Invalid Stage', data: 0 }, // Fixed to number
];
...
RAW_FUNNEL_DATA.forEach((item) => {
  ...
  if (typeof item.key !== 'string') {
    item.key = String(item.key || 'Unknown Stage'); // Ensure key is a string
  }
});
const FUNNEL_CHART_DATA: FunnelDataPoint[] = RAW_FUNNEL_DATA;

const IncidentFunnelReportCard: React.FC<{ data?: FunnelDataPoint[], title?: string }> = ({ data, title = "Incident Report" }) => {
  const funnelAxisLineColor = '#7E7E8F75'; // Semi-transparent gray for theme neutrality
  ...
  const chartData = data || FUNNEL_CHART_DATA;

  return (
    <div className="flex flex-col justify-between pt-4 pb-4 bg-white dark:bg-black rounded-3xl shadow-xl w-full h-full overflow-hidden text-black dark:text-white transition-colors duration-300 border border-hairline">
      <div className="flex justify-between items-center p-7 pt-6 pb-8">
        <h3 className="text-2xl text-left font-bold tracking-tight">
          {title}
        </h3>
        <select
          aria-label="Select time range"
          className="p-2 rounded-md bg-gray-100 text-gray-800 dark:bg-[#262631] dark:text-white text-xs transition-colors duration-300 focus:outline-none"
        >
        ...
        <FunnelChart
          ...
          series={
            <FunnelSeries
              arc={
                <FunnelArc
                  colorScheme={['#EE409430', '#EE409475', '#EE4094']} // Specific layered colors
                  gradient={null}
                  variant="layered" // Key for layered appearance
                  glow={{
                    blur: 30,
                    color: '#EE409499', // Specific glow color
                  }}
                />
              }
              axis={
                <FunnelAxis
                  label={
                    <FunnelAxisLabel className="font-bold text-[10px] text-gray-600 dark:text-gray-400 transition-colors duration-300" />
                  }
      ...
      <div className="flex w-full pl-8 pr-8 justify-between pb-2 pt-3">
        <div className="flex flex-col gap-1 w-1/2">
          <span className="text-sm font-light text-muted-slate uppercase tracking-wider">Active Records</span>
          <div className="flex items-center gap-2">
            <CountUp
              ...
              duration={2.5}
            />
            <div className="flex bg-red-500/10 p-1 pl-2 pr-2 items-center rounded-full text-red-600 text-[10px] transition-colors duration-300">
              <UpArrowIcon strokeColor="currentColor" className="w-3 h-3" />
              12%
            </div>
          </div>
        </div>
        <div className="flex flex-col gap-1 w-1/2">
          <span className="text-sm font-light text-muted-slate uppercase tracking-wider">Total Records</span>
          <div className="flex items-center gap-2">
            <CountUp
              className="font-mono text-3xl font-semibold"
              start={0}
              ...
              duration={2.5}
            />
            <div className="flex bg-teal-500/10 p-1 pl-2 pr-2 items-center rounded-full text-teal-600 text-[10px] transition-colors duration-300">
              <DownArrowIcon strokeColor="currentColor" className="w-3 h-3" />
              4%
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-col pl-8 pr-8 font-mono divide-y divide-hairline transition-colors duration-300">
        {metricItems.map((item, index) => (
          <motion.div
            ...
            className="flex w-full pb-3 pt-3 items-center gap-2"
          >
            <div className="flex flex-row gap-2 items-center text-xs w-1/2 text-gray-600 dark:text-[#9A9AAF] transition-colors duration-300">
              {React.cloneElement(item.icon as React.ReactElement, { fill: item.changeType === 'increase' && index < 2 ? 'rgb(232,64,69)' : 'rgb(64,229,209)' })}
            ...
            <div className="flex gap-2 w-1/2 justify-end items-center">
              ...
              <div
                className={`flex p-1 pl-2 pr-2 items-center rounded-full text-[10px] ${
...
export default IncidentFunnelReportCard;
